...
t_color	ft_specular(t_lht *now, t_ray ray)
{
	...
	reflect = ft_vec_sub(ft_vec_mult_const(*ray.hit_norm, 2 * ft_dot_product(*ray.hit_norm, dir)), dir);
...
t_color	ft_get_light_color(t_lht *now, t_object *objs, t_ray ray)
{
	...
	temp_ray = ft_ray_init(ft_vec_dup(ft_vec_add(*ray.hit_point, ft_vec_mult_const(*ray.hit_norm, 1))), dir);
